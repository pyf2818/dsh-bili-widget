...
function apply(ctx) {
  ...
  const cookieJar = path.join(homedir(), '.dsh', 'tmp-bili-cookies.txt')
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        else if (kind === 'upper-videos') api = 'http://api.bilibili.com/x/web-interface/search/type?search_type=video&keyword=' + encodeURIComponent(q.get('name') || '') + '&mid=' + encodeURIComponent(q.get('mid') || '') + '&page=' + (q.get('pn') || 1)
        ...
        const raw = (kind === 'search' || kind === 'upper-videos') ? (obj.data.result || []) : (kind === 'follow-videos' ? (obj.data.articles || []) : (obj.data.list || []))
