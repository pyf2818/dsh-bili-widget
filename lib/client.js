...
.dy-root{position:fixed;top:0;left:0;right:0;bottom:0;pointer-events:none;z-index:2147483000;font-family:'Segoe UI',system-ui,sans-serif}
.dy-launch{position:fixed;top:0;left:0;transform:translate(-50%,-50%);pointer-events:auto!important;cursor:grab;touch-action:none;user-select:none;width:34px;height:34px;border-radius:50%;border:none;background:#fb7299;color:#fff;box-shadow:0 2px 8px rgba(251,114,153,.5);display:flex;align-items:center;justify-content:center;z-index:2147483647;transition:background .15s}
.dy-launch:hover{background:#e85c92}
...
.dy-launch.dy-minimized{animation:dy-pulse 1.6s ease-in-out infinite;background:#ff2d78;box-shadow:0 0 0 4px rgba(255,45,120,.2)}
@keyframes dy-pulse{0%,100%{box-shadow:0 0 0 4px rgba(255,45,120,.2)}50%{box-shadow:0 0 0 8px rgba(255,45,120,.5)}}
.dy-launch svg{display:block}
.dy-win{position:fixed;pointer-events:auto;background:#fff;border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,.22);display:flex;flex-direction:column;overflow:hidden;z-index:2147483100;will-change:left,top,width,height}
.dy-win--mini{box-shadow:0 4px 16px rgba(0,0,0,.25);border-radius:10px}
.dy-minimized-win{visibility:hidden!important;pointer-events:none!important}
.dy-hideable{display:none!important}
.dy-header{display:flex;align-items:center;gap:6px;padding:8px 10px;background:linear-gradient(90deg,#fb7299,#e83d7c);color:#fff;cursor:move;user-select:none;flex:none;touch-action:none}
.dy-brand{font-weight:800;font-size:14px;white-space:nowrap}
.dy-back{border:none;background:rgba(255,255,255,.22);color:#fff;border-radius:10px;padding:3px 8px;font-size:11px;cursor:pointer;white-space:nowrap;flex:none}
.dy-back:hover{background:rgba(255,255,255,.38)}
.dy-search{flex:1;min-width:0;display:flex;gap:4px}
.dy-search input{flex:1;min-width:0;border:none;border-radius:14px;padding:4px 10px;font-size:12px;outline:none}
.dy-hbtn{border:none;background:rgba(255,255,255,.22);color:#fff;border-radius:12px;padding:4px 8px;font-size:12px;cursor:pointer;white-space:nowrap}
.dy-hbtn:hover{background:rgba(255,255,255,.38)}
.dy-tabs{display:flex;gap:4px;padding:6px 8px 0;flex:none}
.dy-tab{border:none;background:transparent;color:#555;font-size:13px;padding:4px 10px;border-radius:10px;cursor:pointer}
.dy-tab:hover{background:#f5f5f5}
.dy-tab.on{background:#fb7299;color:#fff;font-weight:700}
.dy-tab.ai{color:#a855f7}
.dy-tab.ai.on{background:linear-gradient(135deg,#a855f7,#ec4899);color:#fff}
.dy-chips{display:flex;gap:6px;padding:6px 8px;overflow-x:auto;flex:none;scrollbar-width:none}
.dy-chip{flex:none;border:1px solid #e3e3e3;background:#fff;color:#666;border-radius:12px;padding:2px 10px;font-size:11px;cursor:pointer;white-space:nowrap}
.dy-chip:hover,.dy-chip.on{border-color:#fb7299;color:#fb7299;background:#fff0f5}
.dy-chips.ai .dy-chip:hover,.dy-chips.ai .dy-chip.on{border-color:#a855f7;color:#a855f7;background:#faf5ff}
.dy-opts{display:flex;gap:14px;align-items:center;padding:4px 10px;flex:none;font-size:12px;color:#666}
.dy-opt{display:flex;align-items:center;gap:5px;cursor:pointer;user-select:none}
.dy-toggle{display:inline-flex;width:34px;height:18px;border-radius:9px;background:#ddd;position:relative;transition:background .2s;flex:none}
.dy-toggle.on{background:#fb7299}
.dy-toggle::after{content:'';position:absolute;left:2px;top:2px;width:14px;height:14px;border-radius:50%;background:#fff;transition:left .2s}
.dy-toggle.on::after{left:18px}
...
.dy-mini-ctrl button{border:none;background:rgba(0,0,0,.55);color:#fff;border-radius:10px;padding:3px 9px;font-size:11px;cursor:pointer}
.dy-nextrow{display:flex;align-items:center;gap:8px;padding:4px 10px 6px;flex:none;font-size:11px;color:#666}
.dy-nextrow .dy-nextinfo{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#999}
.dy-nextrow .dy-nextinfo b{color:#fb7299;font-weight:600}
.dy-meta{display:flex;gap:6px;flex:none}
.dy-mbtn{border:1px solid #e0e0e0;background:#fff;color:#666;border-radius:12px;padding:2px 10px;font-size:11px;cursor:pointer}
.dy-mbtn:hover{border-color:#fb7299;color:#fb7299}
.dy-list{flex:1;overflow-y:auto;padding:0 8px 8px}
.dy-item{display:flex;gap:8px;padding:8px;border-radius:10px;cursor:pointer;align-items:flex-start}
.dy-item:hover{background:#f7f7f7}
.dy-item.playing{background:#fff0f5}
.dy-item img{width:96px;height:60px;object-fit:cover;border-radius:6px;background:#eee;flex:none}
.dy-item .dy-pic-ph{width:96px;height:60px;border-radius:6px;background:linear-gradient(135deg,#f0f0f0,#e3e3e3);flex:none}
.dy-item .dy-info{flex:1;min-width:0}
.dy-item .dy-title{font-size:12px;color:#333;line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
.dy-item .dy-sub{font-size:11px;color:#999;margin-top:4px;display:flex;gap:8px;flex-wrap:wrap}
.dy-follow-btn{margin-left:auto;flex:none;border:1px solid #e0e0e0;background:#fff;color:#666;border-radius:10px;padding:2px 10px;font-size:11px;cursor:pointer;white-space:nowrap}
.dy-follow-btn:hover{border-color:#fb7299;color:#fb7299}
.dy-follow-btn.dy-followed{border-color:#ddd;color:#999;background:#f5f5f5}
.dy-follow-btn.dy-followed:hover{border-color:#c00;color:#c00}
.dy-hist{position:absolute;inset:0;background:#fff;z-index:6;display:flex;flex-direction:column}
.dy-hist-hd{display:flex;align-items:center;gap:8px;padding:8px 10px;border-bottom:1px solid #eee;flex:none}
.dy-hist-hd b{flex:1;font-size:13px}
.dy-hist-list{flex:1;overflow-y:auto;padding:8px}
.dy-help-body{flex:1;overflow-y:auto;padding:12px 14px}
.dy-help-sec{margin-bottom:16px}
.dy-help-sec:last-child{margin-bottom:0}
.dy-help-title{display:flex;align-items:center;gap:6px;font-size:12px;font-weight:700;color:#333;margin-bottom:8px}
.dy-help-row{display:flex;align-items:center;gap:8px;font-size:12px;color:#666;padding:3px 0}
.dy-help-key{flex:none;min-width:22px;text-align:center;border:1px solid #ddd;background:#f7f7f7;border-radius:5px;padding:1px 5px;font-size:11px;font-weight:600;color:#444;box-shadow:0 1px 0 #e0e0e0}
.dy-help-dot{flex:none;width:5px;height:5px;border-radius:50%;background:#fb7299;margin-left:8px}
.dy-help-desc{font-size:12px;color:#666;line-height:1.6;margin-bottom:8px}
.dy-help-link{display:flex;align-items:center;gap:6px;font-size:12px;color:#fb7299;cursor:pointer;padding:4px 0}
.dy-help-link:hover{text-decoration:underline}
.dy-help-author{font-size:11px;color:#aaa;margin-top:8px}
.dy-empty{text-align:center;color:#bbb;font-size:12px;padding:30px 0}
.dy-skel{display:flex;gap:8px;padding:8px;align-items:flex-start}
.dy-skel-img{width:96px;height:60px;border-radius:6px;background:linear-gradient(135deg,#e8e8e8,#f0f0f0);flex:none;animation:dy-shimmer 1.5s infinite}
.dy-skel-body{flex:1;display:flex;flex-direction:column;gap:6px}
.dy-skel-line{height:10px;border-radius:5px;background:linear-gradient(135deg,#e8e8e8,#f0f0f0);animation:dy-shimmer 1.5s infinite}
.dy-skel-line.short{width:60%}
@keyframes dy-shimmer{0%{opacity:.6}50%{opacity:1}100%{opacity:.6}}
.dy-watched{position:relative}
.dy-watched::after{content:'已看';position:absolute;top:2px;right:2px;background:rgba(0,0,0,.55);color:#ccc;font-size:9px;padding:1px 4px;border-radius:4px;line-height:1.2}
.dy-retry{display:flex;gap:8px;align-items:center;justify-content:center;padding:12px}
.dy-retry-btn{border:1px solid #fb7299;color:#fb7299;background:#fff;border-radius:10px;padding:4px 16px;font-size:12px;cursor:pointer}
.dy-retry-btn:hover{background:#fff0f5}
.dy-hist .dy-hbtn{background:#f2f2f2;color:#666;border:1px solid #e0e0e0}
.dy-hist .dy-hbtn:hover{background:#e6e6e6;color:#333}
.dy-resize{position:absolute;right:0;bottom:0;width:16px;height:16px;cursor:se-resize;z-index:8;touch-action:none}
.dy-resize::after{content:'';position:absolute;right:3px;bottom:3px;width:7px;height:7px;border-right:2px solid #bbb;border-bottom:2px solid #bbb}
			...
			function Widget() {
				...
				react.useEffect(function () {
					function onKey(e) {
						...
						if (e.key === 'ArrowRight' || e.key === 'n' || e.key === 'N') { e.preventDefault(); nextPlay(); return }
						if (e.key === 'm' || e.key === 'M') { e.preventDefault(); setSt(function (x) { return Object.assign({}, x, { uiMode: x.uiMode === 'mini' ? 'normal' : 'mini' }) }); return }
						if (e.key === 'f' || e.key === 'F') { e.preventDefault(); setSt(function (x) { return Object.assign({}, x, { uiMode: x.uiMode === 'video' ? 'normal' : 'video' }) }); return }
						if (e.key === 'ArrowLeft' || e.key === 'Backspace') { e.preventDefault(); goBack(); return }
						if (e.key === 'Escape') {
				...
				el('div', { className: 'dy-help-body' },
					el('div', { className: 'dy-help-sec' },
						...
						el('div', { className: 'dy-help-row' }, el('span', { className: 'dy-help-key' }, '→'), el('span', { className: 'dy-help-key' }, 'N'), el('span', null, '播放下一个')),
						el('div', { className: 'dy-help-row' }, el('span', { className: 'dy-help-key' }, 'M'), el('span', null, '迷你模式')),
						el('div', { className: 'dy-help-row' }, el('span', { className: 'dy-help-key' }, 'F'), el('span', null, '专注模式')),
						el('div', { className: 'dy-help-row' }, el('span', { className: 'dy-help-key' }, '←'), el('span', null, '返回上一页')),
						el('div', { className: 'dy-help-row' }, el('span', { className: 'dy-help-key' }, 'Esc'), el('span', null, '关闭窗口'))
					),
					el('div', { className: 'dy-help-sec' },
						el('div', { className: 'dy-help-title' },
							el('svg', { viewBox: '0 0 24 24', width: '15', height: '15', fill: 'none', stroke: '#fb7299', strokeWidth: '2', strokeLinecap: 'round', strokeLinejoin: 'round', 'aria-hidden': 'true' },
