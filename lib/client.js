		...
		function apply(ctx) {
			...
			function Widget() {
				...
				const helpPanel = helpView ? el('div', { className: 'dy-hist' },
					el('div', { className: 'dy-hist-hd' },
						el('b', null, '⌨️ 快捷键'),
						el('button', { className: 'dy-hbtn', onClick: function () { setHelpView(false) } }, '返回')
					),
					el('div', { className: 'dy-hist-list', style: { fontSize: '12px', color: '#666', lineHeight: '2' } },
						el('div', null, '→ 或 N', el('span', { style: { color: '#bbb', marginLeft: '6px' } }, '播放下一个')),
						el('div', null, 'M', el('span', { style: { color: '#bbb', marginLeft: '6px' } }, '切换迷你模式')),
						el('div', null, 'F', el('span', { style: { color: '#bbb', marginLeft: '6px' } }, '切换专注模式')),
						el('div', { style: { marginTop: '12px', borderTop: '1px solid #eee', paddingTop: '8px', color: '#999', fontSize: '11px' } },
							'小贴士：视频播放中可随时最小化到悬浮球（— 按钮），视频会继续播放'
						)
					)
				) : null
