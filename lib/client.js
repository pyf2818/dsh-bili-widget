window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function clampPos(s) {
			...
			let x = s.x, y = s.y
			if (typeof x !== 'number' || isNaN(x) || x < 8 || x > vw - 60) x = vw - w - 16
...
`
		...
		function apply(ctx) {
			...
			// Host HTTP 路由: /api/bili2/fetch 和 /api/bili2/pic
			...
			const DEFAULT = { tab: 'rec', topic: '', cur: null, items: [], page: 1, done: false, autoplay: false, endMode: 'next', uiMode: 'normal', w: 420, h: 640, x: 0, y: 0, bx: 0, by: 0, kw: '', region: 0 }
			...
			function Widget() {
				...
				function fetchArgs(tab, topic, region, page) {
					if (tab === 'search') return { kind: 'search', keyword: topic || '', page: page, search_type: s.searchType || 'video' }
					if (tab === 'rec') {
						if (topic) return { kind: 'search', keyword: topic, page: page }
						return { kind: 'hot', pn: page }
					}
					if (tab === 'hot') return { kind: 'rank', rid: 0 }
					if (tab === 'rank') return { kind: 'rank', rid: region || 0 }
				...
				function doFetch(gen, tab, topic, region, page, append, key) {
					...
					apiFetch(args).then(function (res) {
						...
						const items = res.list.map(function (v) {
							return { bvid: v.bvid || '', aid: v.aid || 0, title: stripTags(v.title), pic: v.pic || '', duration: v.duration != null ? v.duration : 0, author: v.author || '', play: v.play || 0, danmaku: v.danmaku || 0, pubdate: v.pubdate || 0 }
						})
						setSt(function (s) {
			...
			function doSearch() {
				...
				function closeWin() { setHistView(false); setOpen(false) }
				function reopen() {
					setOpen(true)
				...
				function toggleWin() { if (open) closeWin(); else reopen() }
				...
				const header = el('div', { className: 'dy-header' + (hideMain ? ' dy-hideable' : ''), onPointerDown: startDrag },
					...
					el('div', { className: 'dy-search' },
						...
						el('button', { className: 'dy-hbtn', onClick: consume(closeWin) }, '关闭')
					)
				)

				const tabs = el('div', { className: 'dy-tabs' + (hideMain ? ' dy-hideable' : '') },
					[['rec', '推荐'], ['hot', '热门'], ['rank', '排行'], ['pick', '精选'], ['ai', 'AI']].map(function (t) {
						return el('button', { key: t[0], className: 'dy-tab' + (t[0] === 'ai' ? ' ai' : '') + (st.tab === t[0] && !st.topic ? ' on' : ''), onClick: consume(function () { openTab(t[0]) }) }, t[1])
					})
				)
			...
			const chipsRow = chips.length ? el('div', { className: 'dy-chips' + (st.tab === 'ai' ? ' ai' : '') + (hideMain ? ' dy-hideable' : ''), onWheel: function (e) { if (e.deltaY) e.currentTarget.scrollLeft += e.deltaY } },
				...
				const list = el('div', { className: 'dy-list' + (hideMain ? ' dy-hideable' : ''), onScroll: function (e) {
					...
					}) : el('div', { className: 'dy-empty' }, errMsg ? el('div', { className: 'dy-retry' }, el('div', null, errMsg), el('button', { className: 'dy-retry-btn', onClick: consume(function () { loadList(st.tab, st.topic, st.region, 1, false) }) }, '重试')) : (st.tab === 'search' ? '无结果，换个词试试' : '列表为空'))
				)
				...
				const histPanel = histView ? el('div', { className: 'dy-hist' },
					...
					el('div', { className: 'dy-hist-list' },
						hist.length ? hist.map(function (h, i) {
				...
				return el('div', { className: 'dy-root' },
					el('button', { className: 'dy-launch', onPointerDown: onBallDown, title: 'B站', style: { left: bp.x + 'px', top: bp.y + 'px' } },
					...
					open ? el('div', { ref: winRef, className: winCls, style: winStyle() },
						header, tabs, kwHistRow, chipsRow, opts, player, nextRow, playerStatus, list, histPanel,
