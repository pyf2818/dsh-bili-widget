		...
		function apply(ctx) {
			...
			function Widget() {
				...
				function fetchArgs(tab, topic, region, page) {
					...
					if (tab === 'follow') return { kind: 'upper-videos', mid: topic || '', pn: page }
				...
				function doFetch(gen, tab, topic, region, page, append, key) {
					...
					apiFetch(args).then(function (res) {
						...
						const items = res.list.map(function (v) {
							return { bvid: v.bvid || '', aid: v.aid || 0, title: stripTags(v.title), pic: v.pic || '', duration: v.duration != null ? v.duration : 0, author: v.author || '', play: v.play || 0, danmaku: v.danmaku || 0, pubdate: v.pubdate || 0, mid: v.mid || 0 }
						})
						setSt(function (s) {
				...
				function toggleFollow(v) {
					...
					if (isFollowed(v.mid)) {
						...
						apiFollowAdd({ mid: v.mid, name: v.author || v.title, face: v.pic || '' }).then(function () {
							setFollows(follows.concat([{ mid: v.mid, name: v.author || v.title, face: v.pic || '', latest_bvid: v.bvid || '' }]))
						})
					}
				}
				...
				function closeWin() { setHistView(false); setOpen(false); setSt(function (x) { return Object.assign({}, x, { minimized: false }) }) }
				function minimizeWin() { setOpen(false); setSt(function (x) { return Object.assign({}, x, { minimized: true }) }) }
				function reopen() {
					setOpen(true)
				...
				function startDrag(e) {
					...
					function onMove(ev) {
						...
						if (win) {
							win.style.left = (fromX + dx) + 'px'
							win.style.top = (fromY + dy) + 'px'
						}
					}
					function onUp() {
						document.removeEventListener('pointermove', onMove)
						...
						if (win) {
							setSt(function (s) { return Object.assign({}, s, { x: parseInt(win.style.left, 10) || s.x, y: parseInt(win.style.top, 10) || s.y }) })
						}
					}
					document.addEventListener('pointermove', onMove)
					document.addEventListener('pointerup', onUp)
				...
				const hideMain = (st.uiMode === 'mini' || st.uiMode === 'video' || histView)
				const hidePlayer = histView
				...
				const list = el('div', { className: 'dy-list' + (hideMain ? ' dy-hideable' : ''), onScroll: function (e) {
					...
					st.items.length ? st.items.map(function (v, i) {
						...
						const followBtn = isFollowTab ? el('button', { className: 'dy-follow-btn' + (isFollowed(v.mid) ? ' dy-followed' : ''), onClick: consume(function (e) { e.stopPropagation(); toggleFollow(v) }) }, isFollowed(v.mid) ? '已关注' : '+ 关注') : null
				...
				return el('div', { className: 'dy-root' },
					...
					open ? el('div', { ref: winRef, className: winCls, style: winStyle() },
						header, tabs, kwHist, chipsRow, opts, player, nextRow, playerStatus, list, histPanel,
